...
if [[ ${1:-} == '--kill' ]]; then
    ...
    if [[ $line == '[session] '* ]]; then
        tmux kill-session -t "=${line#'[session] '}" 2>/dev/null || true
    fi
    exit 0
fi
